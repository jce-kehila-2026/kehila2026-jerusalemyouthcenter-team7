        ...
        {items.map((item) => {
          ...
          return (
            <View key={item.label} style={chartSt.barCol}>
              {/* Value on top */}
              <Text style={[chartSt.valLabel, { color: item.color }]}>
                {item.value}
              </Text>
              ...
              <Ionicons
                ...
                size={13}
                color={item.color}
                style={chartSt.barIcon}
              />
              <Text style={[chartSt.barLabel, { color: theme.subtext }]}>
                {item.label}
              </Text>
            </View>
          );
    ...
    const fetchStatic = async () => {
      try {
        ...
        if (notifSnap.status === "fulfilled" && notifSnap.value.size > 0)
          setNotifList(
            notifSnap.value.docs.map((d) => ({
              id: d.id,
              ...(d.data() as Omit<DashNotif, "id">),
            })),
          );
