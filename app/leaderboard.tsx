    ...
    <View style={[p.col, rank === 1 && p.colCenter]}>
      ...
      {entry ? (
        <View style={p.avatarWrap}>
          ...
          >
            <Text style={[p.avatarText, { fontSize: avatarSizes[rank - 1] * 0.38 }]}>
        ...
        <View style={p.avatarWrap}>
          <View style={[p.avatar, { width: avatarSizes[rank - 1], height: avatarSizes[rank - 1], borderRadius: avatarSizes[rank - 1] / 2, borderColor: "#ddd" }]}>
...
const p = StyleSheet.create({
  ...
  blockRank: { fontSize: 18, fontWeight: "900", color: "rgba(255,255,255,0.7)" },
});
      ...
      <View style={s.header}>
        ...
        <View style={s.headerRow}>
          ...
          <View style={s.headerCenter}>
            ...
            <Text style={s.headerSub}>Who's singing the most? 🎵</Text>
          </View>
          {/* My rank bubble */}
        ...
        {myEntry && (
          <View style={s.myStrip}>
            <Text style={s.myStripText}>
              🌟 You have <Text style={s.myStripPts}>{myEntry.points} points</Text> — keep going!
            </Text>
          </View>
        )}
      </View>
      ...
      >
        ...
        {leaderboard.length === 0 && (
          <View style={s.emptyHero}>
            ...
            <Text style={s.emptySub}>Complete challenges below to be the first on the board 🚀</Text>
          </View>
        )}
            ...
            {restList.map((entry, i) => {
              ...
              return (
                <View key={entry.uid} style={[s.rankRow, isMe && s.rankRowMe]}>
                  <View style={[s.rankNumBadge, isMe && { backgroundColor: TEAL }]}>
                    <Text style={[s.rankNum, isMe && { color: "#fff" }]}>#{rank}</Text>
                  </View>
                  <View style={[s.rankAvatar, isMe && { borderColor: TEAL, backgroundColor: TEAL + "20" }]}>
                  ...
                  <View style={{ flex: 1, marginLeft: 10 }}>
                    ...
                    <Text style={s.rankVoice}>
                      {VOICE_EMOJI[entry.voice_type?.toLowerCase()] ?? "🎵"} {entry.voice_type ?? ""}
                    </Text>
                  </View>
                  <View style={[s.ptsBadge, isMe && { backgroundColor: TEAL + "20", borderColor: TEAL }]}>
        ...
        <View style={s.challengesHeader}>
          ...
          <View>
            <Text style={s.challengesTitle}>THIS WEEK'S MISSIONS</Text>
        ...
        {challenges.length === 0 ? (
          <View style={s.emptyChall}>
            ...
            <Text style={s.emptyChallText}>No active missions this week yet.</Text>
          ...
          <View style={s.challengeGrid}>
            {challenges.map((ch, i) => (
              <View
                key={i}
                style={[s.challengeCard, { backgroundColor: CHALLENGE_COLORS[i % CHALLENGE_COLORS.length] }]}
              >
                ...
                <Text style={s.challengeCardLabel} numberOfLines={3}>{ch.label}</Text>
              </View>
            ))}
          </View>
        )}
...
const s = StyleSheet.create({
  ...
  myBubble: {
    ...
    borderRadius: 14, paddingHorizontal: 10,
    paddingVertical: 6, alignItems: "center",
    borderWidth: 1.5, borderColor: "rgba(255,255,255,0.4)",
    minWidth: 48,
  },
  myBubbleRank: { fontSize: 18, fontWeight: "900", color: "#fff" },
  myBubbleLabel: { fontSize: 9, color: "rgba(255,255,255,0.8)", fontWeight: "700" },
  myStrip: {
    backgroundColor: "rgba(255,255,255,0.15)",
    borderRadius: 12, paddingHorizontal: 14,
    paddingVertical: 8, marginTop: 12,
  },
  myStripText: { fontSize: 12, color: "#fff", fontWeight: "600", textAlign: "center" },
  ...
  podiumBase: {
    height: 10, backgroundColor: "#e8eef2",
  ...
  rankRow: {
    ...
    padding: 12, borderWidth: 1.5, borderColor: "#e8eef2",
  },
  rankRowMe: {
    borderColor: TEAL, backgroundColor: TEAL + "06",
  },
  rankNumBadge: {
  ...
  challengesHeader: {
    flexDirection: "row", alignItems: "center",
    gap: 12, marginHorizontal: 16, marginTop: 28, marginBottom: 14,
  },
  challengesEmoji: { fontSize: 36 },
  challengesTitle: {
    fontSize: 16, fontWeight: "900", color: DARK, letterSpacing: 0.8,
  },
  challengesSub: { fontSize: 12, color: MUTED, marginTop: 2, fontWeight: "600" },
  ...
  challengeCardDecor: {
    position: "absolute",
    top: 10, right: 12,
    fontSize: 40, color: "rgba(255,255,255,0.2)",
  },
  challengeCardPts: {
