...
function AreaTrendChart({ values, labels, color, chartWidth }: {
  values: number[]; labels: string[]; color: string; chartWidth: number;
}) {
  const H = 175;
  const PL = 30, PR = 8, PT = 10, PB = 32;
  ...
  if (values.length < 2) {
    return (
      <View style={{ height: H, alignItems: 'center', justifyContent: 'center' }}>
        <Text style={{ color: B.muted, fontSize: 13 }}>Not enough data</Text>
      </View>
    );
  }
  ...
  return (
    <Svg width={chartWidth} height={H}>
      ...
      {[0, 25, 50, 75, 100].map(v => {
        ...
        return (
          <G key={v}>
            <Line x1={PL} y1={y} x2={chartWidth - PR} y2={y} stroke={B.border} strokeWidth="1" />
            <SvgText x={PL - 4} y={y + 4} fontSize="9" fill={B.muted} textAnchor="end">{v}</SvgText>
          </G>
        );
      })}
      ...
      {pts.map((p, i) => (
        <G key={i}>
          <Circle cx={p.x} cy={p.y} r="4" fill={B.card} stroke={color} strokeWidth="2" />
          <SvgText x={p.x} y={H - 6} fontSize="9" fill={B.muted} textAnchor="middle">{labels[i]}</SvgText>
        </G>
      ))}
    </Svg>
  );
...
// ── Chart: Vertical bar ────────────────────────────────────────────────────────
function BarChart({ values, labels, color, accentMax = false, accentColor, chartWidth, chartHeight = 140 }: {
  values: number[]; labels: string[]; color: string; accentMax?: boolean; accentColor?: string;
  chartWidth: number; chartHeight?: number;
}) {
  const PL = 26, PR = 6, PT = 10, PB = 26;
  const PW = chartWidth - PL - PR;
    ...
    <Svg width={chartWidth} height={chartHeight}>
      {[0, 50, 100].map(pct => {
        ...
        return (
          <G key={pct}>
            <Line x1={PL} y1={y} x2={chartWidth - PR} y2={y} stroke={B.border} strokeWidth="1" />
            <SvgText x={PL - 3} y={y + 4} fontSize="8" fill={B.muted} textAnchor="end">{v}</SvgText>
          </G>
        );
      })}
      {values.map((v, i) => {
        ...
        return (
          <G key={i}>
            ...
            {barH > 14 && (
              <SvgText x={x + barW / 2} y={y - 3} fontSize="8" fill={B.sub} textAnchor="middle">{v}</SvgText>
            )}
            {(n <= 7 || i % 2 === 0) && (
              <SvgText x={x + barW / 2} y={chartHeight - 4} fontSize="8" fill={B.muted} textAnchor="middle">{labels[i]}</SvgText>
            )}
          </G>
        );
      })}
...
// ── Chart: Donut ───────────────────────────────────────────────────────────────
function DonutChart({ female, male, size = 120 }: { female: number; male: number; size?: number }) {
  const total = female + male;
  if (total === 0) return null;
  const cx = size / 2, cy = size / 2;
  const outerR = size / 2 - 6, innerR = outerR * 0.6;
  ...
  return (
    <Svg width={size} height={size}>
      ...
      <SvgText x={cx} y={cy - 4} fontSize="17" fontWeight="bold" fill={B.text} textAnchor="middle">
        {Math.round((female / total) * 100)}%
      </SvgText>
      <SvgText x={cx} y={cy + 13} fontSize="10" fill={B.muted} textAnchor="middle">Female</SvgText>
    </Svg>
  );
}

// ── Brand Card ─────────────────────────────────────────────────────────────────
function BrandCard({ barColor = B.teal, title, children, style }: {
  barColor?: string; title?: string; children: React.ReactNode; style?: object;
}) {
  return (
...
const bc = StyleSheet.create({
  ...
  bar: { height: 4 },
  pad: { padding: 16 },
  title: { fontSize: 11, fontWeight: '700', color: B.muted, letterSpacing: 1, marginBottom: 12 },
});

// ── KPI card (no inner title) ──────────────────────────────────────────────────
function KpiCard({ icon, accent, label, main, sub }: {
  icon: React.ComponentProps<typeof Ionicons>['name'];
  accent: string; label: string; main: string; sub?: string;
}) {
  return (
...
const kp = StyleSheet.create({
  ...
  bar:   { height: 4 },
  pad:   { padding: 16 },
  icon:  { width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  label: { fontSize: 11, color: B.muted, fontWeight: '600', marginBottom: 4 },
  main:  { fontSize: 22, fontWeight: '900', color: B.text },
  sub:   { fontSize: 11, color: B.muted, marginTop: 2 },
});
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
      {options.map(opt => (
        ...
        >
          <Text style={[fc.text, opt.value === value && fc.textActive]}>{opt.label}</Text>
        </Pressable>
      ))}
    </ScrollView>
  );
...
const fc = StyleSheet.create({
  chip:       { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 24, backgroundColor: '#edf2f5', marginRight: 8, borderWidth: 1.5, borderColor: 'transparent' },
  active:     { backgroundColor: B.teal + '15', borderColor: B.teal },
  text:       { fontSize: 13, color: B.muted, fontWeight: '600' },
  textActive: { color: B.teal, fontWeight: '700' },
});
...
export default function StatisticsScreen() {
  ...
  const groupGenderData = useMemo(() =>
    STAT_GROUPS.map(g => {
      ...
      return { ...g, female: gs.filter(s => s.gender === 'female').length, male: gs.filter(s => s.gender === 'male').length, total: gs.length };
    }), []);
  ...
  const { trendValues, trendLabels } = useMemo(() => {
    ...
    if (selYear === 'all') {
      const vals = Array.from(STAT_YEARS).map(y => {
        const e = MONTHLY_ATTENDANCE.find(m => m.year === y && m.groupId === gKey);
        if (!e) return 0;
        const active = e.rates.filter(r => r > 0);
        return active.length ? Math.round(active.reduce((a, b) => a + b, 0) / active.length) : 0;
      });
      return { trendValues: vals, trendLabels: Array.from(STAT_YEARS).map(String) };
    }
  ...
  const avgAttendance = useMemo(() => {
    ...
    return Math.round(trendValues.reduce((a, b) => a + b, 0) / trendValues.length);
  }, [trendValues]);

  const yoyData = useMemo(() => ({
    values: Array.from(STAT_YEARS).map(y => YEARLY_TOTALS[y]?.students ?? 0),
  ...
  const sessionData = useMemo(() => {
    ...
    return { values: raw.filter(v => v > 0), labels: MONTH_SHORT.filter((_, i) => raw[i] > 0) };
  }, [selYear]);

  const totalHours = useMemo(() => {
    if (selYear === 'all') return Object.values(YEARLY_TOTALS).reduce((acc, v) => acc + v.hours, 0);
    return YEARLY_TOTALS[parseInt(selYear, 10)]?.hours ?? 0;
  }, [selYear]);

  const fPct = genderData.total > 0 ? Math.round((genderData.female / genderData.total) * 100) : 0;
  ...
  return (
    <View style={s.screen}>
      ...
      <View style={s.filterBar}>
        <FilterChips options={YEAR_OPTS}  value={selYear}  onChange={setSelYear} />
        <View style={{ height: 8 }} />
        <FilterChips options={GROUP_OPTS} value={selGroup} onChange={setSelGroup} />
      </View>
      ...
      >
        ...
        <View style={[s.row, { marginTop: 8 }]}>
          <KpiCard icon="female" accent={F_COLOR} label="Female" main={String(genderData.female)} sub={`${fPct}% of group`} />
          <KpiCard icon="male"   accent={M_COLOR} label="Male"   main={String(genderData.male)}   sub={`${mPct}% of group`} />
        </View>
        ...
        <View style={{ marginTop: 16 }}>
          <BrandCard
            barColor={B.teal}
            title={selYear === 'all' ? 'Yearly avg attendance (%)' : `Monthly attendance — ${selYear} (%)`}
          >
            <AreaTrendChart
              values={trendValues}
              labels={trendLabels}
        ...
        {/* Demographics row */}
        <View style={[s.row, { marginTop: 8 }]}>
          <BrandCard barColor={B.teal} title="Gender Split" style={{ width: halfW }}>
            <View style={{ alignItems: 'center' }}>
              <DonutChart female={genderData.female} male={genderData.male} size={108} />
              <View style={s.legend}>
                <View style={s.lgItem}>
          ...
          <BrandCard barColor={B.yellow} title="By Group" style={{ width: halfW }}>
            {groupGenderData.map(g => {
              ...
              return (
                <View key={g.id} style={{ marginBottom: 8 }}>
                  ...
                  <View style={s.gBar}>
                    <View style={[s.gSeg, { flex: frac || 0.01,       backgroundColor: F_COLOR }]} />
                    <View style={[s.gSeg, { flex: (1 - frac) || 0.01, backgroundColor: M_COLOR }]} />
                  </View>
                  <Text style={s.gSub}>{g.female}F · {g.male}M</Text>
                </View>
              );
            })}
          </BrandCard>
        ...
        <View style={[s.row, { marginTop: 8 }]}>
          <BrandCard barColor={B.teal} title="YoY Growth" style={{ width: halfW }}>
            <BarChart
          ...
          <BrandCard barColor={B.yellow} title="Sessions / Month" style={{ width: halfW }}>
            {sessionData.values.length > 0 ? (
              ...
              <View style={{ height: 100, alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ color: B.muted, fontSize: 12 }}>No data</Text>
              </View>
            )}
          </BrandCard>
        </View>
...
const s = StyleSheet.create({
  ...
  lgTxt:   { fontSize: 11, color: B.sub },

  // Group bars
  gLabel: { fontSize: 10, color: B.text, fontWeight: '700', marginBottom: 3 },
  gBar:   { flexDirection: 'row', height: 7, borderRadius: 4, overflow: 'hidden', backgroundColor: B.border, marginBottom: 2 },
  gSeg:   { height: 7 },
  gSub:   { fontSize: 10, color: B.muted },
});
